...
local function notify(src, key, notifyType, ...)
    TriggerClientEvent('ox_lib:notify', src, {
        title = 'Lenhador', description = locale(key, ...), type = notifyType or 'inform'
    })
end
...
local function registerItems()
    for name, data in pairs(Config.Items) do
        if not RSGCore.Shared.Items[name] then
            ...
            if not ok then print(('[fipas-lumberjack] falha ao registrar %s: %s'):format(name, reason)) end
        end
    end
end
...
lib.callback.register('fipas-lumberjack:server:begin', function(source, data)
    if sessions[source] then return false, 'already_chopping' end
    ...
    if not player or not player.Functions.GetItemByName(Config.Tool) then return false, 'no_axe' end
    local key = nodeKey(coords)
    if cooldowns[key] and cooldowns[key] > os.time() then return false, 'tree_depleted' end
...
RegisterNetEvent('fipas-lumberjack:server:cancel', function(token)
    local src = source
    if sessions[src] and sessions[src].token == token then sessions[src] = nil end
end)

RegisterNetEvent('fipas-lumberjack:server:complete', function(token)
    local src = source
    ...
    if not player or not player.Functions.GetItemByName(Config.Tool) then return notify(src, 'no_axe', 'error') end
    if cooldowns[session.key] and cooldowns[session.key] > os.time() then return notify(src, 'tree_depleted', 'error') end
    ...
    if not player.Functions.AddItem(reward.name, amount, false, false, 'fipas-lumberjack') then return notify(src, 'inventory_full', 'error') end
    ...
    if math.random(1, 100) <= Config.ToolBreakChance then
        player.Functions.RemoveItem(Config.Tool, 1, false, 'fipas-lumberjack-tool-break')
        notify(src, 'axe_broke', 'error')
    end
end)
...
CreateThread(function()
    for _, model in ipairs(Config.Trees) do validModels[joaat(model)] = true end
    for _, reward in ipairs(Config.Rewards) do totalWeight = totalWeight + reward.weight end
    registerItems()
    print(('[fipas-lumberjack] iniciado com %s modelos de arvore e %s recompensas.'):format(#Config.Trees, #Config.Rewards))
end)
