...
Get-ChildItem -LiteralPath $CfxSource -Force | ForEach-Object {
    Copy-Item -LiteralPath $_.FullName -Destination $CfxTarget -Recurse -Force
}
...
if (-not $SkipOxMySQL) {
    ...
    $DownloadDir = Join-Path $ProjectRoot 'artifacts'
    $ZipPath = Join-Path $DownloadDir 'oxmysql.zip'
    $Standalone = Join-Path $ProjectRoot 'resources/[standalone]'
    $OxTarget = Join-Path $Standalone 'oxmysql'
    $ExtractPath = Join-Path $DownloadDir 'oxmysql-extracted'
